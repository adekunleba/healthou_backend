...
export default class Server {
  ...
  public static async start(): Promise<Hapi.Server> {
    try {
      ...
      Logger.info(
        `Server - Up and running at http://${config.app.host}:${config.app.port}`
      );
      Logger.info(
        `Server - Visit http://${config.app.host}:${config.app.port}/api/users for REST API`
      );
      Logger.info(
        `Server - Visit http://${config.app.host}:${config.app.port}/documentation for Swagger docs`
      );
